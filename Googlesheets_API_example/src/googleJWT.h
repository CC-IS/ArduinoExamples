...
char* createJWT() {
    ...
    static char base64Payload[700];
    base64url_encode(
        (unsigned char *)payload,  // Data to encode.
        strlen(payload),           // Length of data to encode.
        base64Payload);            // Base64 encoded data.

    static char headerAndPayload[800];
    sprintf(headerAndPayload, "%s.%s", base64Header, base64Payload);
    ...
    rc = mbedtls_md(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), (const unsigned char*)headerAndPayload, strlen(headerAndPayload), digest);
    if (rc != 0) {
    ...
    //MBEDTLS_PK_SIGNATURE_MAX_SIZE default is 512, so we use that for obuf
    if (rc != 0) {
        printf("Failed to mbedtls_pk_sign: %d (-0x%x): %s\n", rc, -rc, mbedtlsError(rc));
        return nullptr;
    }

    static char base64Signature[700];
    base64url_encode((unsigned char *)oBuf, retSize, base64Signature);

    char* retData = (char*)malloc(strlen(headerAndPayload) + 1 + strlen(base64Signature) + 1);
